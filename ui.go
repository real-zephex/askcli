...
func printSearchFilesCall(req searchFilesRequest) {
	lines := []string{toolStyle.Render("↳ tool: search_files")}
	lines = append(lines, subtleStyle.Render(fmt.Sprintf("pattern: %s", req.Pattern)))
	lines = append(lines, subtleStyle.Render(fmt.Sprintf("root: %s", req.Root)))
	lines = append(lines, subtleStyle.Render(fmt.Sprintf("max_results: %d", req.MaxResults)))
	fmt.Println(renderToolBlock(lines))
}

func printSearchFilesResult(result searchFilesResult) {
	if result.ExecutionErr != "" {
		line := subtleStyle.Render(fmt.Sprintf("tool result: error • %s", result.ExecutionErr))
		fmt.Println(renderToolBlock([]string{line}))
		return
	...
	line := subtleStyle.Render(fmt.Sprintf("tool result: ok • %d file(s)%s", len(result.Files), truncMsg))
	fmt.Println(renderToolBlock([]string{line}))
}

func printGrepFilesCall(req grepFilesRequest) {
	lines := []string{toolStyle.Render("↳ tool: grep_files")}
	lines = append(lines, subtleStyle.Render(fmt.Sprintf("pattern: %s", req.Pattern)))
	lines = append(lines, subtleStyle.Render(fmt.Sprintf("root: %s", req.Root)))
	if req.Include != "" {
		lines = append(lines, subtleStyle.Render(fmt.Sprintf("include: %s", req.Include)))
	}
	lines = append(lines, subtleStyle.Render(fmt.Sprintf("max_results: %d", req.MaxResults)))
	fmt.Println(renderToolBlock(lines))
}

func printGrepFilesResult(result grepFilesResult) {
	if result.ExecutionErr != "" {
		line := subtleStyle.Render(fmt.Sprintf("tool result: error • %s", result.ExecutionErr))
		fmt.Println(renderToolBlock([]string{line}))
		return
	...
	line := subtleStyle.Render(fmt.Sprintf("tool result: ok • %d match(es)%s", len(result.Matches), truncMsg))
	fmt.Println(renderToolBlock([]string{line}))
}
